<script>
	...
	json_label_aboutus = <?php echo json_encode($label_aboutus) ?>;

	function load_label_aboutus(label_aboutus){
		var html="";
		for(var i = 0; i < label_aboutus.length; i++){
			var a = '<tr>\n\
						<td>'+(i+1)+'</td>\n\
                        <td>'+label_aboutus[i].position+'</td>\n\
						<td>'+label_aboutus[i].label_en+'</td>\n\
						<td>'+label_aboutus[i].label_id+'</td>\n\
						<td>\n\
            ...
            <div class="modal-body">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="hidden" class="form-control" id="id_label_aboutus">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="box-body">
                    <center>
                        <input type="hidden" id="hapus_id_label_aboutus">
                        <p style="font-size: 15px;">Do you really want to delete <span id="hapus_nama_label_aboutus" style="font-weight: bold;"></span> ?</p>
                    </center>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
            <h3 class="box-title">Data Label About Us</h3>
...
load_label_aboutus(json_label_aboutus);

function showAdd(){
	$("#id_modal").modal("show");
	$("#id_label_aboutus").val("");
    $("#position").val("");    
	$("#label_en").val("");
	$("#label_id").val("");
}
...
function showEdit(i){
	$("#id_label_aboutus").val(json_label_aboutus[i].id_label_aboutus);
    $("#position").val(json_label_aboutus[i].position);    
	$("#label_en").val(json_label_aboutus[i].label_en);
	$("#label_id").val(json_label_aboutus[i].label_id);
	$("#id_modal").modal("show");	
}
...
$("#form_model").submit(function (e){
		...
		$.ajax({
			url: "<?php echo base_url().'admin/mst_label_aboutus/insert_label'; ?>",
			data: { "id_label_aboutus":$("#id_label_aboutus").val(),
			...
			success: function (data, textStatus, jqXHR) {
                ...
                $("#load_form_modal").fadeOut(1000, function (){
                    ...
                    $("#notif_form_modal").fadeOut(2000, function (){
                        json_label_aboutus=data.label_aboutus;
                        load_label_aboutus(json_label_aboutus);
                        $("#id_modal").modal("hide");
                    });
                });
...
function showDelete(i){
	$("#hapus_id_label_aboutus").val(json_label_aboutus[i].id_label_aboutus);
	$("#hapus_nama_label_aboutus").html(json_label_aboutus[i].label_en);
...
$("#form_model_del").submit(function (e){
        ...
        $.ajax({
            url: "<?php echo base_url().'admin/mst_label_aboutus/delete_label'; ?>",
            data: {"id_label_aboutus":$("#hapus_id_label_aboutus").val()},
            type: 'POST',
            dataType: 'json',
            success: function (data, textStatus, jqXHR) {
