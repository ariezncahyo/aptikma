<script>
	...
	json_services_homepage = <?php echo json_encode($services_homepage) ?>;

	function load_services_homepage(services_homepage){
		var html="";
		for(var i = 0; i < services_homepage.length; i++){
            if(services_homepage[i].status=="1") 
                var status = "Active"; 
            else
			...
			var a = '<tr>\n\
						<td>'+(i+1)+'</td>\n\
						<td>'+services_homepage[i].title_en+'</td>\n\
						<td>'+services_homepage[i].title_id+'</td>\n\
                        <td>'+services_homepage[i].desc_en+'</td>\n\
                        <td>'+services_homepage[i].desc_id+'</td>\n\
                        <td>'+status+'</td>\n\
						<td>\n\
            ...
            <div class="modal-body">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="hidden" class="form-control" id="id_services_homepage">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="box-body">
                    <center>
                        <input type="hidden" id="hapus_id_services_homepage">
                        <p style="font-size: 15px;">Do you really want to delete <span id="hapus_title_services_homepage" style="font-weight: bold;"></span> ?</p>
                    </center>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
            <h3 class="box-title">Data Services Homepage</h3>
...
load_services_homepage(json_services_homepage);

function showAdd(){
	$("#id_modal").modal("show");
	$("#id_services_homepage").val("");
	$("#title_en").val("");
	$("#title_id").val("");
    $("#desc_en").val("");
    $("#desc_id").val("");
...
function showEdit(i){
	$("#id_services_homepage").val(json_services_homepage[i].id_services_homepage);
	$("#title_en").val(json_services_homepage[i].title_en);
	$("#title_id").val(json_services_homepage[i].title_id);
    $("#desc_en").val(json_services_homepage[i].desc_en);
    $("#desc_id").val(json_services_homepage[i].desc_id);
    $("#icon").val(json_services_homepage[i].icon);
    $("#status").val(json_services_homepage[i].status);
	$("#id_modal").modal("show");	
}
...
$("#form_model").submit(function (e){
		...
		$.ajax({
			url: "<?php echo base_url().'admin/mst_services_homepage/insert_services_homepage'; ?>",
			data: { "id_services_homepage":$("#id_services_homepage").val(),
			...
			success: function (data, textStatus, jqXHR) {
                ...
                $("#load_form_modal").fadeOut(1000, function (){
                    ...
                    $("#notif_form_modal").fadeOut(2000, function (){
                        json_services_homepage=data.services_homepage;
                        load_services_homepage(json_services_homepage);
                        $("#id_modal").modal("hide");
                    });
                });
...
function showDelete(i){
	$("#hapus_id_services_homepage").val(json_services_homepage[i].id_services_homepage);
	$("#hapus_title_services_homepage").html(json_services_homepage[i].title_en);
...
$("#form_model_del").submit(function (e){
        ...
        $.ajax({
            url: "<?php echo base_url().'admin/mst_services_homepage/delete_services_homepage'; ?>",
            data: {"id_services_homepage":$("#hapus_id_services_homepage").val()},
            type: 'POST',
            dataType: 'json',
            success: function (data, textStatus, jqXHR) {
