<script>
	...
	json_tag_blog = <?php echo json_encode($tag_blog) ?>;

	function load_tag_blog(tag_blog){
		var html="";
		for(var i = 0; i < tag_blog.length; i++){
			var a = '<tr>\n\
						<td>'+(i+1)+'</td>\n\
						<td>'+tag_blog[i].name+'</td>\n\
						<td>\n\
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Insert/Update Tag</h4>
            </div>
            <form id="form_model" class="form-horizontal">
            <div class="modal-body">
                <div class="box-body">       
                    <div class="form-group">
                        <label for="inputEmail3" class="col-sm-4 control-label">Tag Name</label>
                        <div class="col-sm-8">
                            <input type="hidden" class="form-control" id="id_tag_blog">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="box-body">
                    <center>
                        <input type="hidden" id="hapus_id_tag_blog">
                        <p style="font-size: 15px;">Do you really want to delete <span id="hapus_nama_tag_blog" style="font-weight: bold;"></span> ?</p>
                    </center>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
            <h3 class="box-title">Data Tag Blog</h3>
            <button class="btn btn-xs btn-primary pull-right" data-toggle="tooltip" title="Add Category" style="margin-right: 5px;"  onclick="showAdd()"><i class="fa fa-plus"></i></button>
        </div><!-- /.box-header -->
        <div class="box-body">
...
function showEdit(i){
	$("#id_tag_blog").val(json_tag_blog[i].id_tag_blog);
	$("#name").val(json_tag_blog[i].name);
	$("#id_modal").modal("show");	
}
...
$("#form_model").submit(function (e){
		...
		$.ajax({
			url: "<?php echo base_url().'admin/mst_tag_blog/insert_tag'; ?>",
			data: { "id_tag_blog":$("#id_tag_blog").val(),
					"name":$("#name").val()
				},
			type : 'POST',
			dataType : 'json',
...
function showDelete(i){
	$("#hapus_id_tag_blog").val(json_tag_blog[i].id_tag_blog);
	$("#hapus_nama_tag_blog").html(json_tag_blog[i].name);
...
$("#form_model_del").submit(function (e){
        ...
        $.ajax({
            url: "<?php echo base_url().'admin/mst_tag_blog/delete_tag'; ?>",
            data: {"id_tag_blog":$("#hapus_id_tag_blog").val()},
            type: 'POST',
            dataType: 'json',
            success: function (data, textStatus, jqXHR) {
