<script>
	...
	json_vission_homepage = <?php echo json_encode($vission_homepage) ?>;

	function load_vission_homepage(vission_homepage){
		var html="";
		for(var i = 0; i < vission_homepage.length; i++){
			var a = '<tr>\n\
						<td>'+(i+1)+'</td>\n\
                        <td>'+vission_homepage[i].desc_en+'</td>\n\
                        <td>'+vission_homepage[i].desc_id+'</td>\n\
                        <td>'+vission_homepage[i].image+'</td>\n\
						<td>\n\
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Insert/Update Vission</h4>
            </div>
            ...
            <div class="modal-body">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="hidden" class="form-control" id="id_vission_homepage">
...
<div class="modal fade" id="id_modal_del" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Delete Vission</h4>
            </div>
            <form id="form_model_del" class="form-horizontal">
            <div class="modal-body">
                <div class="box-body">
                    <center>
                        <input type="hidden" id="hapus_id_vission_homepage">
                        <p style="font-size: 15px;">Do you really want to delete <span id="hapus_desc_vission_homepage" style="font-weight: bold;"></span> ?</p>
                    </center>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
            <h3 class="box-title">Data Vission Homepage</h3>
...
load_vission_homepage(json_vission_homepage);

function showAdd(){
	$("#id_modal").modal("show");
	$("#id_vission_homepage").val("");
    $("#desc_en").val("");
    $("#desc_id").val("");
    $("#image").val("");
}
...
function showEdit(i){
	$("#id_vission_homepage").val(json_vission_homepage[i].id_vission_homepage);
    $("#desc_en").val(json_vission_homepage[i].desc_en);
    $("#desc_id").val(json_vission_homepage[i].desc_id);
    $("#image").val(json_vission_homepage[i].image);
	$("#id_modal").modal("show");	
}
...
$("#form_model").submit(function (e){
		...
		$.ajax({
			url: "<?php echo base_url().'admin/mst_vission_homepage/insert_vission_homepage'; ?>",
			data: { "id_vission_homepage":$("#id_vission_homepage").val(),
                    "desc_en":$("#desc_en").val(),
                    "desc_id":$("#desc_id").val(),
                    "image":$("#image").val()
				},
			...
			success: function (data, textStatus, jqXHR) {
                ...
                $("#load_form_modal").fadeOut(1000, function (){
                    ...
                    $("#notif_form_modal").fadeOut(2000, function (){
                        json_vission_homepage=data.vission_homepage;
                        load_vission_homepage(json_vission_homepage);
                        $("#id_modal").modal("hide");
                    });
                });
...
function showDelete(i){
	$("#hapus_id_vission_homepage").val(json_vission_homepage[i].id_vission_homepage);
	$("#hapus_desc_vission_homepage").html(json_vission_homepage[i].desc_en);
...
$("#form_model_del").submit(function (e){
        ...
        $.ajax({
            url: "<?php echo base_url().'admin/mst_vission_homepage/delete_vission_homepage'; ?>",
            data: {"id_vission_homepage":$("#hapus_id_vission_homepage").val()},
            type: 'POST',
            dataType: 'json',
            success: function (data, textStatus, jqXHR) {
